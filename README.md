...
- **依訊號避險**：下跌機率越高 → 避險力道越大 → 黃金曝險越低。
  避險部位視為被中性化（毛報酬 ≈ 0），現實中最接近「賣出等量黃金期貨鎖價」。
- 比較兩者的**累積損益曲線**與**最大回撤**，並逐年列出情境表現。

### 避險成本（不能省略的一塊）

現實中避險要花錢。回測把成本拆成兩項，逐日從毛報酬扣除（見 `src/backtest.py`
的 `_apply_costs()`）：

| 項目 | 算法 | 對應現實 |
|------|------|----------|
| 持有成本 carry | 避險比例 × 年化成本 ÷ 252 | 轉倉價差、保證金機會成本、選擇權權利金 |
| 調倉成本 turnover | \|避險比例變動\| × 單邊交易成本 | 手續費、滑價、買賣價差 |

預設年化 0.3%（COMEX 期貨）＋單邊 0.02%，可在 `config.py` 的 `HEDGE_COST_ANNUAL`
/ `HEDGE_TC_ONEWAY` 調整。同時保留**未扣成本的毛曲線**當對照 —— 兩條線的差距
就是這份保險的實際價格。

`cost_sweep()` 進一步掃過五種避險工具的成本（期貨 / GLD / 黃金存摺 / 保護性賣權），
輸出**保護單價**：每換到 1 個百分點的最大回撤改善，要放棄幾個百分點的總報酬。

> **實測結論（誠實揭露）**：本回測期間避險把最大回撤從 -44.4% 改善到 -41.8%
> （+2.6pp），代價是總報酬從 378% 降到 326%（-52pp），**保護單價約 20 倍**。
> 由於模型平均只降低 9% 曝險、一年換手 0.4 次，成本本身不是瓶頸 ——
> 即使假設保險完全免費，保護單價仍有 19 倍。也就是說：黃金長期走多的期間，
> 把曝險關掉的代價遠大於它擋掉的跌幅，這套訊號式避險並不划算。
...
- `outputs/backtest.json`
  - `equity_curve`：有/無避險累積損益、避險比例，以及未扣成本的 `hedge_gross` 對照。
  - `stats`：總報酬、年化、波動、Sharpe、最大回撤（含 `hedge_gross` 毛口徑）。
  - `cost`：避險成本明細（年化假設、累計付出、平均避險比例、年換手次數）。
  - `cost_sweep`：五種避險工具的「報酬讓出 vs 回撤改善 vs 保護單價」權衡表。
